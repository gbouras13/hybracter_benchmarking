...
rule hybracter_hybrid_8_threads_real:
    ...
    shell:
        '''
        hybracter hybrid-single -l {input.l} -1 {input.short_one} -2 {input.short_two} --no_medaka  -c {params.chromosome} -s {wildcards.sample} -o {params.out_dir} -t {threads}  --skip_qc
        '''
